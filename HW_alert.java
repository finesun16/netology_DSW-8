 ...
 * каждый раз, когда общая стоимость покупок продукта за минуту было больше, чем MAX_PRODUCT_PRICE_PER_MINUTE
 */
public class HW_AlertsApp {
    public static final String DATA_FROM_TOPIC_NAME = "purchase_with_joined_product-dsl";
    public static final String DATA_TO_TOPIC = "purchase_with_joined_product_alert-dsl";

    private static final double MAX_PRODUCT_PRICE_PER_MINUTE = 3000D;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        var serDeProps = Map.of(
                ...
                KafkaAvroSerializerConfig.AUTO_REGISTER_SCHEMAS, "true", KafkaAvroSerializerConfig.SCHEMA_REGISTRY_URL_CONFIG, "http://localhost:8081");
    ...
    public static Properties getStreamsConfig() {
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "ProductPriceAlertsAppDSL");
    ...
    public static Topology buildTopology(SchemaRegistryClient client, Map<String, String> serDeConfig) {
        ...
        var purchasesStream = builder.stream(DATA_FROM_TOPIC_NAME, // указываем имя топика
                Consumed.with(new Serdes.StringSerde(), avroSerde) // указываем тип ключа и тип значения в топике
        );

        Duration oneMinute = Duration.ofMinutes(1);
        purchasesStream.groupBy(
                        (key, val) -> val.get("product_id").toString(),
                        Grouped.with(new Serdes.StringSerde(), avroSerde)
                ).windowedBy(
                ...
                .filter(
                        (key, val) -> val > MAX_PRODUCT_PRICE_PER_MINUTE
                ).toStream().map(
                        (key, val) -> {
                            // создаем схему нашего алерта
                            Schema schema = SchemaBuilder.record("ProductPriceAlert")
                                    .fields()
                                    .name("window_start")
                            ...
                            record.put("product_price", val);
                            return KeyValue.pair(key.key(), record);
                        }).to(DATA_TO_TOPIC, Produced.with(new Serdes.StringSerde(), avroSerde));
